...
class FlexibleEarthquakeCNN(nn.Module):
    ...
    def __init__(self, n_layers, n_filters, kernel_size, dropout_rate, fc_size, fc_size2, use_batchnorm):
...
def objective(trial):
    ...
    best_model_path = DATA_DIR / "best_hyperparameter_model.pth"
    ...
    if not best_model_path.exists():
        ...
        current_best = torch.load(best_model_path, map_location="cpu")

        if best_rmse < current_best["val_rmse"]:
            save_model = True
    ...
    if save_model:
        checkpoint = {
            ...
            "val_mae": best_mae,
            "val_mse": best_mse,
            "val_accuracy_0.2": best_accuracy,
            "trainable_params": trainable_params
        }

        torch.save(checkpoint, best_model_path)

        print(f"New overall best model saved | Trial {trial.number} | RMSE: {best_rmse:.4f}")
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        ...
        storage="sqlite:///" + str(DATA_DIR / "hyperparameter_search2.db"),
        direction="minimize",
        load_if_exists=True,
    ...
    results_df.to_csv(DATA_DIR / "hyperparameter_search_results2.csv",index=False)
