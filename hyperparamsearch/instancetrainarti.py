...
class FlexibleEarthquakeCNN(nn.Module):
    ...
    def __init__(self, n_layers, n_filters, kernel_size, dropout_rate, fc_size, fc_size2, use_batchnorm, fc_size3=None):
        ...
        classifier = [
            ...
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(fc_size, fc_size2),
            nn.ReLU(),
            nn.Dropout(dropout_rate)
        ]

        if fc_size3 is not None:
            classifier.append(nn.Linear(fc_size2, fc_size3))
            classifier.append(nn.ReLU())
            classifier.append(nn.Dropout(dropout_rate))
            classifier.append(nn.Linear(fc_size3, 1))
        else:
            classifier.append(nn.Linear(fc_size2, 1))
...
def run_experiment(model, train_loader, val_loader, device, num_epochs=40, lr=0.001, weight_decay=1e-4, checkpoint_path=DATA_DIR / "Baseline_Instance_params1.pth"):
    ...
    for epoch in range(num_epochs):
        ...
        train_mse, train_mae, train_accuracy = train_epoch(
            model,
            train_loader,
            loss_function,
            optimizer,
...
def objective(trial):
    ...
    if architecture == "32_6_256_128":
        initial_filters = 32
        n_layers = 6
        fc_size = 256
        fc_size2 = 128

    elif architecture == "16_6_512_128":
        initial_filters = 16
        n_layers = 6
        fc_size = 512
        fc_size2 = 128
    ...
    add_fc_layer = trial.suggest_categorical("add_fc_layer",[True, False])

    if add_fc_layer:
        fc_size3 = trial.suggest_categorical("fc_size3",[32, 64, 128])
    else:
        fc_size3 = None
    ...
    print(f"FC size 2: {fc_size2}")
    print(f"FC size 3: {fc_size3}")
    print(f"Learning rate: {learning_rate}")
    print(f"Weight decay: {weight_decay}")
    print("Pooling: MaxPool1d")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        ...
        storage="sqlite:///" + str(DATA_DIR / "hyperparameter_search.db"),
        direction="minimize",
        load_if_exists=True,
    ...
    results_df.to_csv(
        DATA_DIR / "hyperparameter_search_results.csv",
        index=False
    )
