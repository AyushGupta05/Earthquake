...
class FlexibleEarthquakeCNN(nn.Module):
    ...
    def __init__(self, n_layers, n_filters, kernel_sizes, dropout_rate, fc_size,use_batchnorm):
...
def objective (trial):
    ...
    for i in range(n_layers):
        ...
        n_filters.append(trial.suggest_categorical(f"n_filters_layer_{i}",[16, 32, 64, 128, 256]))

        kernel_sizes.append(trial.suggest_categorical(f"kernel_size_layer_{i}",[3, 5, 7]))
    use_batchnorm = trial.suggest_categorical("use_batchnorm",[True, False])
    dropout_rate = trial.suggest_float("dropout_rate",0.0,0.5)
    fc_size = trial.suggest_categorical("fc_size",[128, 256,512,1024])
    print(f"\n{'='*60}")
